...
function Splash({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    const drawStar8 = (cx: number, cy: number, r: number, time: number, a: number) => {
      ctx.save(); ctx.translate(cx, cy); ctx.rotate(time * 0.00008);
      ctx.globalAlpha = a;
      for (let i = 0; i < 8; i++) {
        ...
        ctx.lineTo(Math.cos(mid) * r * 0.38, Math.sin(mid) * r * 0.38);
        ctx.lineTo(Math.cos(next) * r, Math.sin(next) * r);
        ctx.strokeStyle = `rgba(201,168,76,${0.15 + 0.08 * Math.sin(time * 0.001 + i)})`;
        ctx.lineWidth = 0.8; ctx.stroke();
      }
      for (let r2 = 1; r2 <= 3; r2++) {
        ctx.beginPath(); ctx.arc(0, 0, r * r2 * 0.28, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(201,168,76,${0.05 - r2 * 0.012})`; ctx.lineWidth = 0.5; ctx.stroke();
      }
      ctx.restore();
    };
    ...
    const render = () => {
      ...
      bg.addColorStop(0, '#0a1028'); bg.addColorStop(0.5, '#0c1430'); bg.addColorStop(1, '#080e20');
      ctx.fillStyle = bg; ctx.fillRect(0, 0, w, h);

      // Soft radial glows
      [[w*0.5, h*0.35, 'rgba(201,168,76,0.05)'], [w*0.3, h*0.7, 'rgba(129,140,248,0.03)'], [w*0.8, h*0.6, 'rgba(52,211,153,0.02)']].forEach(([cx, cy, col]) => {
        const g = ctx.createRadialGradient(cx as number, cy as number, 0, cx as number, cy as number, w * 0.45);
      ...
      const patterns = [
        {x:w*0.5, y:h*0.42, r:180}, {x:w*0.15, y:h*0.2, r:70}, {x:w*0.85, y:h*0.18, r:55},
        {x:w*0.2, y:h*0.75, r:65}, {x:w*0.8, y:h*0.78, r:80}, {x:w*0.5, y:h*0.12, r:45},
      ];
      patterns.forEach((p, i) => {
        const fadeIn = Math.min(1, elapsed / (600 + i * 250));
        drawStar8(p.x, p.y, p.r, elapsed + i * 1000, fadeIn * 0.05);
      });
      ...
      for (let i = 0; i < 80; i++) {
        ...
        const tw = 0.15 + 0.85 * Math.sin(elapsed * 0.0015 * (rng() * 0.5 + 0.5) + rng() * 6.28);
        ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(201,168,76,${tw * 0.15})`; ctx.fill();
      }

      // Expanding rings from center
      for (let i = 0; i < 3; i++) {
        const prog = ((elapsed * 0.00025 + i * 0.33) % 1);
        const radius = prog * Math.min(w, h) * 0.45;
        ctx.beginPath(); ctx.arc(w / 2, h * 0.42, radius, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(201,168,76,${(1 - prog) * 0.08})`; ctx.lineWidth = 0.8; ctx.stroke();
      }

      // Floating particles
      for (let i = 0; i < 25; i++) {
        const px = (seededRandom(i * 7)()) * w + Math.sin(elapsed * 0.0004 + i) * 25;
        const py = ((seededRandom(i * 13)()) * h * 1.2 - elapsed * 0.012 * (seededRandom(i * 3)() * 0.5 + 0.5)) % (h * 1.1);
        const ap = py < 0 ? py + h * 1.1 : py;
        const pa = 0.08 + 0.12 * Math.sin(elapsed * 0.002 + i * 2);
        ctx.beginPath(); ctx.arc(px, ap, 1 + Math.sin(elapsed * 0.002 + i) * 0.5, 0, Math.PI * 2);
  ...
  return (
    <div className="splash-overlay">
      ...
      <div className="relative z-10 flex flex-col items-center px-8">
        {/* Logo */}
        <div className={`splash-logo mb-6 ${phase >= 1 ? 'opacity-100' : 'opacity-0'}`} style={{ transition: 'opacity 1s ease' }}>
          <div className="splash-logo-glow" />
          <div className="splash-logo-inner">
            <span className="relative z-10 text-5xl">📖</span>
          </div>
          {[0,1,2].map(i => (
            <div key={i} className="splash-ring" style={{ inset: `${-20-i*15}px`, animationDelay: `${i*0.8}s`, borderColor: `rgba(201,168,76,${0.1-i*0.03})` }} />
          ))}
        </div>

        {/* Bismillah */}
        <div className={`mb-5 ${phase >= 1 ? 'opacity-100' : 'opacity-0'}`}
             style={{ transition: 'opacity 1s ease 0.3s', animation: phase >= 1 ? 'textReveal 1s ease 0.4s both' : 'none' }}>
          <p className="text-2xl font-quran" style={{color:'rgba(201,168,76,0.5)'}}>﷽</p>
        </div>

        {/* Title */}
        <div className={`text-center ${phase >= 2 ? 'opacity-100' : 'opacity-0'}`} style={{ transition: 'opacity 0.8s ease' }}>
          <h1 className="text-3xl font-black gradient-text mb-2" style={{ animation: phase >= 2 ? 'textReveal 0.8s ease both' : 'none' }}>
            {t.splashTitle}
          </h1>
          <p className="text-xs font-semibold tracking-widest" style={{color:'rgba(201,168,76,0.3)', animation: phase >= 2 ? 'textReveal 0.6s ease 0.2s both' : 'none'}}>
            {t.splashSub}
          </p>
        </div>

        {/* Verse */}
        <div className={`mt-8 max-w-xs text-center ${phase >= 3 ? 'opacity-100' : 'opacity-0'}`} style={{ transition: 'opacity 1s ease' }}>
          <p className="text-sm font-quran leading-relaxed" style={{ color: 'rgba(201,168,76,0.25)', animation: phase >= 3 ? 'verseFade 3s ease both' : 'none' }}>
            ﴿ إِنَّا نَحْنُ نَزَّلْنَا الذِّكْرَ وَإِنَّا لَهُ لَحَافِظُونَ ﴾
          </p>
        </div>

        {/* Loading indicator */}
        <div className="mt-12 flex items-center gap-1.5" style={{ animation: 'fadeIn 0.5s ease 1.5s both' }}>
          {[0,1,2,3,4].map(i => (
            <div key={i} className="w-1 h-1 rounded-full" style={{ background: `rgba(201,168,76,${0.4 - i*0.05})`, animation: `glow 1.5s ease-in-out ${i*0.12}s infinite` }} />
          ))}
        </div>
      </div>
    </div>
...
function GuideModal({ onClose }: { onClose: () => void }) {
  ...
  const steps = [
    { icon: '🎙️', num: '1', title: t.guideStep1Title || 'Choose Reciter', desc: t.guideStep1Desc || '', color: 'from-amber-500/20 to-amber-600/10', border: 'border-amber-500/20' },
    { icon: '📖', num: '2', title: t.guideStep2Title || 'Choose Surah', desc: t.guideStep2Desc || '', color: 'from-indigo-500/20 to-indigo-600/10', border: 'border-indigo-500/20' },
    { icon: '🎬', num: '3', title: t.guideStep3Title || 'Choose Background', desc: t.guideStep3Desc || '', color: 'from-emerald-500/20 to-emerald-600/10', border: 'border-emerald-500/20' },
    { icon: '✨', num: '4', title: t.guideStep4Title || 'Preview & Generate', desc: t.guideStep4Desc || '', color: 'from-purple-500/20 to-purple-600/10', border: 'border-purple-500/20' },
  ];
  ...
  const tips = [
    ...
    { icon: '👆', text: t.guideTip4 || '' },
    { icon: '📸', text: t.guideTip5 || '' },
  ];

  const extras = [
    { icon: '🔍', title: t.guidePexelsTitle || 'Pexels', desc: t.guidePexelsDesc || '' },
    { icon: '📐', title: t.guideQualityTitle || 'Quality', desc: t.guideQualityDesc || '' },
  ];
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4"
      ...
      {/* Backdrop */}
      <div className="absolute inset-0 bg-black/70 backdrop-blur-md" style={{animation:'fadeIn 0.3s ease'}} />
      
      {/* Modal */}
      <div ref={modalRef} className="relative z-10 w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-3xl border border-white/[0.08]"
           style={{
             background: 'linear-gradient(145deg, rgba(15,23,42,0.97), rgba(10,15,30,0.98))',
             boxShadow: '0 25px 60px rgba(0,0,0,0.5), 0 0 40px rgba(212,168,83,0.05)',
             animation: 'scaleIn 0.3s ease'
           }}>
        
        {/* Header */}
        <div className="sticky top-0 z-10 p-5 pb-4 border-b border-white/[0.06]"
             style={{background:'linear-gradient(145deg, rgba(15,23,42,0.98), rgba(10,15,30,0.99))'}}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-amber-500/15 to-amber-600/10 border border-amber-500/15 flex items-center justify-center">
                <span className="text-xl">📚</span>
              </div>
              <div>
                <h2 className="text-sm font-black gradient-text">{t.guideTitle || 'User Guide'}</h2>
                <p className="text-[9px] text-amber-100/30 font-bold">{t.guideSubtitle || ''}</p>
              </div>
            </div>
            <button onClick={onClose} className="w-7 h-7 rounded-xl bg-white/[0.04] border border-white/[0.08] flex items-center justify-center text-white/30 hover:text-white/60 hover:bg-white/[0.08] transition-all text-xs">
              ✕
            </button>
          </div>
        </div>
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="space-y-3">
            {steps.map((s, i) => (
              <div key={i} className={`flex gap-3 p-3.5 rounded-2xl bg-gradient-to-r ${s.color} border ${s.border}`}
                   style={{animation:`fadeInUp 0.4s ease ${i*0.1}s both`}}>
                <div className="shrink-0 w-10 h-10 rounded-xl bg-white/[0.06] flex items-center justify-center relative">
                  <span className="text-lg">{s.icon}</span>
                  <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-gradient-to-br from-amber-500 to-amber-700 flex items-center justify-center text-[8px] text-white font-black">{s.num}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-amber-200 font-extrabold text-xs mb-1">{s.title}</h3>
                  <p className="text-white/40 text-[10px] leading-relaxed font-semibold">{s.desc}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Extras - Pexels & Quality */}
          <div className="space-y-3">
            {extras.map((ex, i) => (
              <div key={i} className="p-3.5 rounded-2xl bg-white/[0.02] border border-white/[0.05]"
                   style={{animation:`fadeInUp 0.4s ease ${0.4+i*0.1}s both`}}>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-base">{ex.icon}</span>
                  <h3 className="text-amber-200/80 font-extrabold text-xs">{ex.title}</h3>
                </div>
                <p className="text-white/35 text-[10px] leading-relaxed font-semibold">{ex.desc}</p>
              </div>
            ))}
          </div>

          {/* Tips */}
          <div className="p-4 rounded-2xl bg-gradient-to-br from-indigo-500/[0.06] to-purple-500/[0.04] border border-indigo-500/10"
               style={{animation:'fadeInUp 0.4s ease 0.6s both'}}>
            <h3 className="text-indigo-300/80 font-extrabold text-xs mb-3 flex items-center gap-2">
              <span>💡</span> {t.guideTips || 'Tips'}
            </h3>
            <div className="space-y-2.5">
              {tips.map((tip, i) => (
                <div key={i} className="flex items-start gap-2.5">
                  <span className="text-sm shrink-0 mt-0.5">{tip.icon}</span>
                  <p className="text-white/40 text-[10px] leading-relaxed font-semibold">{tip.text}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Close Button */}
          <button onClick={onClose} className="w-full py-2.5 rounded-2xl bg-gradient-to-r from-amber-500/15 to-amber-600/10 border border-amber-500/20 text-amber-300 text-xs font-extrabold hover:from-amber-500/25 hover:to-amber-600/15 transition-all max-h-[36px]">
            {t.guideClose || 'Got it ✓'}
          </button>
        </div>
      </div>
        ...
        <div key={i} className="flex items-center">
          <div className="flex flex-col items-center gap-2">
            <div className={`step-circle ${i<step?'completed':i===step?'active':'inactive'}`}>
              {i<step ? <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7"/></svg> : <span className="text-xs">{icons[i]}</span>}
            </div>
            <span className={`text-[10px] font-bold tracking-wide hidden sm:block ${i<step?'text-emerald-400/50':i===step?'gradient-text':'text-white/15'}`}>{l}</span>
          </div>
          {i<3 && <div className={`step-line w-6 sm:w-14 md:w-24 mx-2 mb-5 ${i<step?'active':'inactive'}`}/>}
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="animate-fadeInUp max-w-xl mx-auto">
      <div className="section-header">
        ...
        <p className="text-[10px] font-bold mb-2" style={{color:'rgba(201,168,76,0.4)'}}>{t.step1Badge}</p>
        <h2 className="text-xl font-black gradient-text">{t.step1Title}</h2>
      </div>
      <div className="relative mb-5">
        <div className={`absolute ${lang === 'ar' || lang === 'ur' ? 'right-3.5' : 'left-3.5'} top-1/2 -translate-y-1/2 text-white/15`}>{I.search}</div>
      ...
      <div className="space-y-1.5 max-h-[55vh] overflow-y-auto pb-4">
        {list.map((r,idx) => (
          <button key={r.id} onClick={()=>onSel(r.id)}
            className={`reciter-card w-full ${sel===r.id?'active':''}`}
            style={{animation:`fadeInUp 0.3s ease ${idx*0.015}s both`}}>
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-base shrink-0 transition-all ${
              sel===r.id ? 'bg-gradient-to-br from-amber-500/10 to-amber-600/5 border border-amber-500/15' : 'bg-white/[0.03] border border-white/[0.05]'
            }`}>{r.flag}</div>
            <div className="flex-1 min-w-0 text-right">
              <div className={`font-bold text-[13px] transition-colors ${sel===r.id ? 'text-amber-400' : 'text-white/70'}`}>{r.name}</div>
              <div className="text-white/25 text-[10px] font-medium">{r.en}</div>
            </div>
            {sel===r.id && (
              <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0" style={{background:'linear-gradient(135deg,var(--gold),#a07a28)',boxShadow:'0 2px 12px rgba(201,168,76,0.3)'}}>
  ...
  return (
    <div className="animate-fadeInUp max-w-xl mx-auto">
      <div className="section-header">
        ...
        <p className="text-[10px] font-bold mb-2" style={{color:'rgba(201,168,76,0.4)'}}>{tr.step2Badge}</p>
        <h2 className="text-xl font-black gradient-text">{tr.step2Title}</h2>
      </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 max-h-[28vh] overflow-y-auto pb-3 mb-6">
        {list.map(x => (
          <button key={x.n} onClick={()=>{onSurah(x.n);onRange(1,Math.min(x.a,10));}}
            ...
            <div className={`font-bold text-[13px] mt-1.5 transition-colors ${surah===x.n?'text-amber-400':'text-white/65'}`}>{x.name}</div>
            <div className="text-white/20 text-[9px] mt-0.5 font-medium">{x.a} آية</div>
          </button>
        ))}
      </div>
      {s && (
        <div className="glass-card p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-black text-amber-400 text-lg">{tr.surah} {s.name}</h3>
              <span className="text-white/30 text-[10px] font-medium">{s.en} • {s.a} {tr.ayah} • {s.t === 'مكية' ? tr.meccan : tr.medinan}</span>
            </div>
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between mb-2">
                <label className="text-white/30 text-xs font-bold">{tr.fromAyah}</label>
                <span className="text-amber-400 font-black text-sm">{from}</span>
              </div>
              <input type="range" min={1} max={s.a} value={from} onChange={e=>{const v=+e.target.value;onRange(v,Math.max(v,to));}} />
            </div>
            <div>
              <div className="flex justify-between mb-2">
                <label className="text-white/30 text-xs font-bold">{tr.toAyah}</label>
                <span className="text-amber-400 font-black text-sm">{to}</span>
              </div>
...
function Step3({ sel, onSel, customUrl, customType, onCustom }: {
  ...
  const [pxDownloading, setPxDownloading] = useState<number|null>(null);
  const [pxDownloadProgress, setPxDownloadProgress] = useState(0);
  const [pxDownloadError, setPxDownloadError] = useState('');
  ...
  // Download media as blob for Canvas compatibility (avoids CORS)
  const downloadAsBlob = async (url: string, id: number): Promise<string> => {
    setPxDownloading(id);
    setPxDownloadProgress(0);
    setPxDownloadError('');
    try {
      ...
      while (true) {
        ...
        if (total > 0) {
          setPxDownloadProgress(Math.round((received / total) * 100));
        } else {
          setPxDownloadProgress(Math.min(95, Math.round(received / 10000)));
        }
      }
      
      const blob = new Blob(chunks);
      const blobUrl = URL.createObjectURL(blob);
      setPxDownloadProgress(100);
      setPxDownloading(null);
      return blobUrl;
    } catch (err) {
      setPxDownloading(null);
      setPxDownloadError(String(err));
      throw err;
    }
  };

  const selectPexelsPhoto = async (photo: PexelsPhoto) => {
    try {
      const blobUrl = await downloadAsBlob(photo.src.portrait, photo.id);
      setPxSelected(photo.id);
      onCustom(blobUrl, 'image');
      onSel('custom');
    } catch (err) {
  ...
  const selectPexelsVideo = async (video: PexelsVideo) => {
    ...
    try {
      const blobUrl = await downloadAsBlob(best.link, video.id);
      setPxSelected(video.id);
      onCustom(blobUrl, 'video');
      onSel('custom');
    } catch (err) {
  ...
  return (
    <div className="animate-fadeInUp max-w-2xl mx-auto">
      ...
      <div className="section-header">
        <div className="section-icon"><span>🎬</span></div>
        <p className="text-[10px] font-bold mb-2" style={{color:'rgba(201,168,76,0.4)'}}>{t.step3Badge}</p>
        <h2 className="text-xl font-black gradient-text">{t.step3Title}</h2>
        <p className="text-xs font-quran mt-2 max-w-sm mx-auto leading-relaxed" style={{color:'rgba(201,168,76,0.2)'}}>{t.step3Verse}</p>
      </div>
      ...
      {tab === 'pexels' && (
        <div className="space-y-3 mb-5 animate-fadeInUp">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {pxPhotos.map((photo, idx) => (
                  <button key={photo.id} onClick={()=>selectPexelsPhoto(photo)}
                    disabled={pxDownloading === photo.id}
                    ...
                    } ${pxDownloading === photo.id ? 'pointer-events-none' : ''}`}
                    ...
                    {/* Downloading overlay */}
                    {pxDownloading === photo.id && (
                      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center z-20">
                        <div className="w-8 h-8 rounded-full border-2 border-white/10 border-t-green-400 animate-spin mb-2" />
                        <span className="text-green-300 text-[9px] font-bold">⬇ {pxDownloadProgress}%</span>
                        <div className="w-3/4 h-1 bg-white/10 rounded-full mt-1.5 overflow-hidden">
                          <div className="h-full bg-green-400/60 rounded-full transition-all" style={{width:`${pxDownloadProgress}%`}} />
                        </div>
                      </div>
                    )}
                    {/* Badge */}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {pxVideos.map((video, idx) => (
                  <button key={video.id} onClick={()=>selectPexelsVideo(video)}
                    disabled={pxDownloading === video.id}
                    className={`relative rounded-xl overflow-hidden border-2 transition-all cursor-pointer group ${
                      pxSelected === video.id ? 'border-purple-400/50 shadow-lg shadow-purple-500/10' : 'border-white/[0.06] hover:border-white/[0.12]'
                    } ${pxDownloading === video.id ? 'pointer-events-none' : ''}`}
                    style={{aspectRatio:'9/14', animation:`fadeInUp 0.4s ease ${idx*0.05}s both`}}>
                    <img src={video.image} alt="" crossOrigin="anonymous" loading="lazy"
                      className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent" />
                    {/* Downloading overlay */}
                    {pxDownloading === video.id && (
                      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center z-20">
                        <div className="w-8 h-8 rounded-full border-2 border-white/10 border-t-purple-400 animate-spin mb-2" />
                        <span className="text-purple-300 text-[9px] font-bold">⬇ {pxDownloadProgress}%</span>
                        <div className="w-3/4 h-1 bg-white/10 rounded-full mt-1.5 overflow-hidden">
                          <div className="h-full bg-purple-400/60 rounded-full transition-all" style={{width:`${pxDownloadProgress}%`}} />
                        </div>
                      </div>
                    )}
                    {/* Badge */}
                    ...
                    {/* Selected check */}
                    {pxSelected === video.id && !pxDownloading && (
          ...
          {pxSelected && !pxDownloading && sel === 'custom' && customUrl && !pxDownloadError && (
            <div className="glass-card p-3 border border-green-500/20 animate-fadeInUp">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-lg bg-green-500/15 flex items-center justify-center shrink-0">
                  <span className="text-sm">✅</span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-green-400/80 text-[10px] font-bold">{t.pexelsDownloaded || 'Background downloaded successfully!'}</p>
                  <p className="text-white/20 text-[8px] font-semibold mt-0.5">{t.pexelsReadyToUse || 'Ready to use in your reel. Go to next step.'}</p>
                </div>
                <span className="text-green-400/50 text-lg">🎬</span>
              </div>
            </div>
          )}
          ...
          {/* Download Error */}
          {pxDownloadError && (
            <div className="glass-card p-3 border border-red-500/20 animate-fadeInUp">
              <div className="flex items-center gap-2">
                <span className="text-base">⚠️</span>
                <div className="flex-1">
                  <p className="text-red-400/80 text-[10px] font-bold">{t.pexelsDownloadError || 'Download failed. Using direct URL instead.'}</p>
                  <p className="text-white/20 text-[8px] font-mono break-all mt-0.5">{pxDownloadError}</p>
                </div>
                <button onClick={()=>setPxDownloadError('')} className="text-white/20 hover:text-white/40 text-xs">✕</button>
              </div>
            </div>
          )}
...
function Step4({ reciter, surah, from, to, bgId, customUrl, customType, onBack }: {
  ...
  return (
    <div className="animate-fadeInUp max-w-5xl mx-auto">
      <div className="section-header">
        <div className="section-icon"><span>✨</span></div>
        <p className="text-[10px] font-bold mb-2" style={{color:'rgba(52,211,153,0.5)'}}>{tr.step4Badge}</p>
        <h2 className="text-xl font-black gradient-text">{tr.step4Title}</h2>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="glass-card-gold p-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-[8px] font-bold uppercase tracking-wider" style={{color:'rgba(201,168,76,0.3)'}}>{tr.infoSurah.replace('📖 ','')}</div>
                  <div className="text-amber-400 font-extrabold text-xs">{sur.name}</div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-base">🎙️</span>
                <div>
                  <div className="text-[8px] font-bold uppercase tracking-wider" style={{color:'rgba(201,168,76,0.3)'}}>{tr.infoReciter.replace('🎙️ ','')}</div>
                  <div className="text-amber-400 font-extrabold text-xs">{rec.name}</div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-base">📏</span>
                <div>
                  <div className="text-[8px] font-bold uppercase tracking-wider" style={{color:'rgba(201,168,76,0.3)'}}>{tr.infoAyahs.replace('📏 ','')}</div>
                  <div className="text-white/60 font-extrabold text-xs">{from} → {to} <span className="text-white/20 text-[9px] font-bold">({to-from+1})</span></div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-base">🎬</span>
                <div>
                  <div className="text-[8px] font-bold uppercase tracking-wider" style={{color:'rgba(201,168,76,0.3)'}}>{tr.infoSize.replace('🎬 ','')}</div>
                  <div className="text-white/60 font-extrabold text-xs">{QUALITY_MAP[quality].w}×{QUALITY_MAP[quality].h}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-xs font-extrabold" style={{color:'rgba(201,168,76,0.4)'}}>{tr.ayahList}</h3>
              <span className="text-[10px] font-bold" style={{color:'rgba(201,168,76,0.25)'}}>{cur+1}/{ayahs.length}</span>
            </div>
            <div className="max-h-[160px] overflow-y-auto space-y-1.5">
              {ayahs.map((txt,i) => (
                <button key={i} onClick={()=>goToAyah(i)}
                  ...
                  <span className="font-quran leading-relaxed line-clamp-2 font-medium" style={{color:'rgba(255,255,255,0.5)'}}>{txt}</span>
                  {preloadStatus[i] && (
  ...
  return (
    <div className="min-h-screen relative" dir={dir}>
      ...
      <div className="relative z-10">
        <header className="glass sticky top-0 z-50" style={{borderBottom:'1px solid rgba(201,168,76,0.04)'}}>
          <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
            <button onClick={()=>setStep(0)} className="flex items-center gap-3 cursor-pointer group transition-all hover:opacity-80">
              <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{background:'linear-gradient(135deg,rgba(201,168,76,0.08),rgba(201,168,76,0.03))',border:'1px solid rgba(201,168,76,0.1)'}}>
                <span className="text-base">📖</span>
              </div>
              <div>
                <h1 className="text-[13px] font-black gradient-text leading-tight">{t.appTitle}</h1>
                <p className="text-[7px] font-semibold tracking-widest" style={{color:'rgba(201,168,76,0.25)'}}>{t.appSubtitle}</p>
              </div>
            </button>
            <div className="flex items-center gap-1.5">
              <button onClick={()=>setShowGuide(true)} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg transition-all max-h-[28px]" style={{color:'rgba(201,168,76,0.35)',fontSize:'10px',fontWeight:700}} title={t.guide || 'Guide'}>
                <span className="text-sm">📚</span>
                <span className="hidden sm:inline">{t.guide || 'دليل'}</span>
              </button>
              <LangSelector />
              {step > 0 && step < 3 && (
                <button onClick={()=>setStep(s=>s-1)} className="px-2.5 py-1 rounded-lg text-[10px] font-bold transition-all max-h-[28px]" style={{color:'rgba(255,255,255,0.2)',background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.04)'}}>
                  {t.back}
                </button>
              )}
            </div>
            ...
            <>
              ...
              <div className="floating-nav floating-nav-next">
                <button onClick={()=>setStep(s=>s+1)} disabled={!canNext()} className="floating-btn floating-btn-next disabled:opacity-20 disabled:cursor-not-allowed disabled:transform-none">
                  <span className="text-[10px] font-bold">{step===2 ? t.preview : t.next}</span>
                </button>
              </div>
              ...
              {step > 0 && (
                <div className="floating-nav floating-nav-back">
                  <button onClick={()=>setStep(s=>s-1)} className="floating-btn floating-btn-back">
                    <span className="text-[10px] font-bold">{t.prev}</span>
                  </button>
                </div>
              )}
            </>
        ...
        <footer className="text-center py-12 space-y-4 mt-8">
          <div className="flex items-center justify-center gap-4 mb-3">
            <div className="h-px w-16" style={{background:'linear-gradient(to right,transparent,rgba(201,168,76,0.1))'}} />
            <span style={{color:'rgba(201,168,76,0.15)',fontSize:'8px'}}>✦ ✦ ✦</span>
            <div className="h-px w-16" style={{background:'linear-gradient(to left,transparent,rgba(201,168,76,0.1))'}} />
          </div>
          <p className="text-sm font-quran tracking-wider" style={{color:'rgba(201,168,76,0.2)'}}>{t.charity}</p>
          <p className="text-[10px]" style={{color:'rgba(255,255,255,0.08)'}}>{t.developer} <span className="gradient-text font-bold text-[11px]">{t.devName}</span></p>
        </footer>
      </div>
    </div>
  );
